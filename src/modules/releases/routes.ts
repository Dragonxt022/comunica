...
import { uploadImagem, uploadPrint } from '../../middlewares/upload.middleware.ts';
...
router.post('/:id', onlySecom, (req: Request, res: Response, next: NextFunction) => {
  uploadPrint(req, res, (err: any) => {
    if (err) return res.status(400).json({ ok: false, error: err.message });
    next();
  });
